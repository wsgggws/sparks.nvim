...
**🎉 粒子物理引擎 | 🔥 Combo 连击系统 | 🔊 沉浸式音效 | 🌈 基于 Treesitter 的智能着色**
...
- **📱 自适应布局**：根据屏幕大小自动调整动画窗口尺寸和位置，完美适配小屏/大屏。
- **🎭 多种动画预设**：
  - `confetti` (彩带) - 默认输入
  - `explode` (爆炸) - 删除字符
  - `matrix` (黑客帝国) - 绿色代码雨
...
require("sparks").setup({
  ...
  -- 外观
  winblend = 0,           -- 窗口透明度(0-100)，默认 0。设置 > 0 以透出光标行背景
})
```
...
**提示**：如果你发现动画窗口背景遮挡了光标行高亮（CursorLine），可以在配置中设置 `winblend = 10` 或更高，来开启透明效果。
...
- 自动清理资源
- 根据屏幕大小自适应调整动画窗口尺寸：
  - 小屏幕 (< 100 列)：16x8 窗口，间隙 3
  - 中等屏幕 (100-150 列)：20x10 窗口，间隙 4
  - 大屏幕 (> 150 列)：26x12 窗口，间隙 5
